...
from spring_amr.dataset import AMRDataset, AMRDatasetTokenBatcherAndLoader, AMRMultiTaskDataset
from spring_amr.modeling_bart import AMRBartForConditionalGeneration
from spring_amr.tokenization_bart import AMRBartMultiGenTokenizer, AMRBartTokenizer, PENMANBartMultiGenTokenizer, PENMANBartTokenizer

def instantiate_model_and_tokenizer_multigen(
        name=None,
        checkpoint=None,
    ...
    if name is None:
        ...
        #name = "/home/emarkle26/.cache/huggingface/hub/models--facebook--bart-large/snapshots/cb48c1365bd826bd521f650dc2e0940aee54720c"
        name = "/home/emarkle26/models/bart-large"
    ...
    if penman_linearization:
        tokenizer = PENMANBartMultiGenTokenizer.from_pretrained(
            tokenizer_name,
        ...
        tokenizer = AMRBartMultiGenTokenizer.from_pretrained(
            tokenizer_name,
            collapse_name_ops=collapse_name_ops,
            use_pointer_tokens=use_pointer_tokens,
            config=config,
    ...
    if from_pretrained:
        ...
        model = AMRBartForConditionalGeneration.from_pretrained(
                 "/home/emarkle26/models/bart-large",
                config=config
                )
    else:
    ...
    print("Tokenizer Size: {}".format(len(tokenizer.encoder)))
...
def instantiate_multitask_loader(
        glob_pattn,
        tokenizer,
        batch_size=500,
        evaluation=True,
    ...
    dataset = AMRMultiTaskDataset(
        ...
        dereify=dereify,
        next_gen=next_gen,
        amr_mask_ratio=amr_mask_ratio,
        nl_mask_ratio=nl_mask_ratio
    )
