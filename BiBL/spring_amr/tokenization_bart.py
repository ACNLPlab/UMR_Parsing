...
class AMRBartMultiGenTokenizer(BartTokenizer):
    ...
    def init_amr_vocabulary(self, pred_min=5):
        ...
        self.gen_token = self.INIT + '<gen>'
        self.mask_token = self.INIT + '<mask>'
        self.gen_tlp_token = self.INIT + '<gentlp>'
        self.ans_token = self.INIT + '<ans>'
        self.gen_token_id = self.convert_tokens_to_ids(self.gen_token)
        self.gen_tlp_token_id = self.convert_tokens_to_ids(self.gen_tlp_token)
        self.ans_token_id = self.convert_tokens_to_ids(self.ans_token)
    ...
    def batch_encode_sentences_new(self, sentences, device=torch.device('cpu')):
        sentences = [s for s in sentences]
        extra = {'sentences': sentences}
        batch = self.batch_encode_plus(sentences, return_tensors='pt', pad_to_max_length=True)
        batch = {k: v.to(device) for k, v in batch.items()}
        return batch, extra
    ...
    def batch_encode_mask(self, sentences, linearized, extras=None, amr_ratio=0.5, nl_ratio=0.5, device=torch.device('cpu')):
        max_lm_len = 500
        sentences = [s for s in sentences]
        ...
        cands = [[x if x != "<mask>" else self.mask_token for x in self.tokenize(sent)] for sent in sentences]
        padded_mask_x, _ = self.batch_encode_sentences(cands, device=device)
        sents_ids = [[id for id in sent if id != self.pad_token_id and id != self.eos_token_id and id != self.bos_token_id] for sent in padded_x["input_ids"]]
        mask_sents_ids = [[id for id in sent if id != self.pad_token_id and id != self.eos_token_id and id != self.bos_token_id] for sent in padded_mask_x["input_ids"]]
        graph_ids = [[id for id in g if id != self.pad_token_id and id != self.eos_token_id and id != self.bos_token_id] for g in linearized]
        mask_graph_ids = [[id for id in g if id != self.pad_token_id and id != self.eos_token_id and id != self.bos_token_id] for g in linearized]
        # add masks to graph_ids
        for index, mask_graph in enumerate(mask_graph_ids):
            mask_num = math.floor(nl_ratio * len(mask_graph))
            rand_ind = random.sample(range(0, len(mask_graph)), mask_num)
            for ind in rand_ind:
                mask_graph[ind] = self.mask_token_id
            mask_graph_ids[index] = mask_graph
        collate_ids = [[self.bos_token_id]+ sents_ids[i] + [self.ans_token_id] +graph_ids[i] + [self.eos_token_id] for i in range(0, len(linearized))]
        rev_collate_ids = [[self.bos_token_id] + graph_ids[i] + [self.ans_token_id] + sents_ids[i] + [self.eos_token_id] for i in range(0, len(linearized))]
        mask_collate_ids = [[self.bos_token_id]+ mask_sents_ids[i] + [self.ans_token_id] + mask_graph_ids[i] + [self.eos_token_id] for i in range(0, len(linearized))]
        mask_rev_collate_ids = [[self.bos_token_id] + mask_graph_ids[i] + [self.ans_token_id] + mask_sents_ids[i] + [self.eos_token_id] for i in range(0, len(linearized))]
        batch = self.batch_encode_plus([f"{self.mask_token}"], return_tensors='pt', pad_to_max_length=True)
        maxlen = 0
        maxmasklen = 0
        fil_collate_ids = []
        fil_rev_collate_ids = []
        mask_fil_sents_ids = []
        mask_fil_graph_ids = []
        mask_fil_collate_ids = []
        mask_fil_rev_collate_ids = []
        for index, collate in enumerate(collate_ids):
            if len(collate) <= max_lm_len:
                ...
                maxmasklen = max((len(mask_collate_ids[index])), maxmasklen)
        ...
        mask_collate_padded = [x + [self.pad_token_id] * (maxmasklen - len(x)) for x in mask_fil_collate_ids]
        mask_rev_collate_padded = [x + [self.pad_token_id] * (maxmasklen - len(x)) for x in mask_fil_rev_collate_ids]
        # print("LM Batch Size:{}, Ori Batch Size: {}".format(len(input_ids), orilen))
        if len(mask_collate_padded) <= 0:
            return {
                "skip_mask": True
            }
        attn_mask = [[1 if id != self.pad_token_id else 0 for id in ids] for ids in mask_collate_padded]
        rev_attn_mask = [[1 if id != self.pad_token_id else 0 for id in ids] for ids in mask_rev_collate_padded]
        collate_padded = torch.tensor(collate_padded).to(device)
        rev_collate_padded = torch.tensor(rev_collate_padded).to(device)
        mask_collate_padded = torch.tensor(mask_collate_padded).to(device)
        mask_rev_collate_padded = torch.tensor(mask_rev_collate_padded).to(device)
        attn_mask = torch.tensor(attn_mask).to(device)
        rev_attn_mask = torch.tensor(rev_attn_mask).to(device)
        batch = {
            'input_ids': mask_collate_padded,
            'rev_input_ids': mask_rev_collate_padded,
            'decoder_input_ids': collate_padded[:, :-1], 
            'lm_labels': collate_padded[:, 1:],
            'rev_decoder_input_ids': rev_collate_padded[:, :-1],
            'rev_lm_labels': rev_collate_padded[:, 1:],
            "attention_mask": attn_mask,
            "rev_attention_mask": rev_attn_mask,
            "skip_mask": False
        }
        return batch

    def batch_encode_lm(self, sentences, linearized, extras=None, device=torch.device('cpu')): 
        # discard length over 500
        max_lm_len = 500
        padded_x, _ = self.batch_encode_sentences(sentences, device=device)
        sents_ids = [[id for id in sent if id != self.pad_token_id and id != self.eos_token_id and id != self.bos_token_id] for sent in padded_x["input_ids"]]
        graph_ids = [[id for id in g if id != self.pad_token_id and id != self.eos_token_id and id != self.bos_token_id] for g in linearized]
        ...
        for index, collate in enumerate(collate_ids):
            if len(collate) <= max_lm_len:
                maxlen = max((len(collate)), maxlen)
                input_ids.append([self.gen_token_id])
                fil_sents_ids.append(sents_ids[index])
                fil_graph_ids.append(graph_ids[index])
                fil_collate_ids.append(collate_ids[index])
                fil_rev_collate_ids.append(rev_collate_ids[index])
        collate_padded = [x + [self.pad_token_id] * (maxlen - len(x)) for x in fil_collate_ids]
        rev_collate_padded = [x + [self.pad_token_id] * (maxlen - len(x)) for x in fil_rev_collate_ids]
        ...
        attn_mask = [[1 if id != self.pad_token_id else 0 for id in ids] for ids in input_ids]
        collate_padded = torch.tensor(collate_padded).to(device)
        rev_collate_padded = torch.tensor(rev_collate_padded).to(device)
        attn_mask = torch.tensor(attn_mask).to(device)
        batch = {
            ...
            'rev_lm_labels': rev_collate_padded[:, 1:],
            "attention_mask": attn_mask,
            "rev_attention_mask": attn_mask,
            "skip_lm": False
        }
        # print(batch)
        return batch
    
    def batch_encode_lm_next_gen(self, sentences, linearized, extras=None, device=torch.device('cpu')): 
        # discard length over 500
        max_lm_len = 500
        padded_x, _ = self.batch_encode_sentences(sentences, device=device)
        sents_ids = [[id for id in sent if id != self.pad_token_id and id != self.eos_token_id and id != self.bos_token_id] for sent in padded_x["input_ids"]]
        ...
        for index, collate in enumerate(collate_ids):
            if len(collate) <= max_lm_len:
                maxlen = max((len(collate)), maxlen)
                input_ids.append([self.gen_tlp_token_id])
                fil_sents_ids.append(sents_ids[index])
                fil_graph_ids.append(graph_ids[index])
                fil_collate_ids.append(collate_ids[index])
                fil_rev_collate_ids.append(rev_collate_ids[index])
...
def decode_amr(self, tokens, restore_name_ops=False):
    # First filter out any problematic tokens that might crash the decoder
    cleaned_tokens = [t for t in tokens if isinstance(t, (int, float))]  # Ensure numeric tokens
    
    try:
        nodes, backreferences = postprocessing.decode_into_node_and_backreferences(cleaned_tokens, self)
    except Exception as e:
        print(f'Decoding failure for tokens: {tokens[:10]}...', file=sys.stderr)
        # Return a minimal valid graph instead of BACKOFF
        return self._create_minimal_graph(tokens), postprocessing.ParsedStatus.OK, ([(':instance', 'unknown')], [])

    if self.use_pointer_tokens:
        try:
            nodes, backreferences = postprocessing.restore_backreferences_from_pointers(nodes)
        except:
            nodes, backreferences = self._simplify_references(nodes)

    try:
        graph_ = graph = postprocessing.build_graph(nodes, backreferences, restore_name_ops=restore_name_ops)
    except Exception as e:
        print(f'Building failure for nodes: {nodes}', file=sys.stderr)
        # Create a linear graph as fallback
        return self._create_linear_graph(nodes), postprocessing.ParsedStatus.OK, (nodes, [])

    try:
        graph, status = postprocessing.connect_graph_if_not_connected(graph)
        if status == postprocessing.ParsedStatus.BACKOFF:
            print('Partial reconnection:', file=sys.stderr)
            # Return the partially connected graph anyway
            return graph, postprocessing.ParsedStatus.OK, (nodes, backreferences)
        return graph, status, (nodes, backreferences)
    except Exception as e:
        print('Reconnection failure, returning partial graph:', file=sys.stderr)
        return graph_, postprocessing.ParsedStatus.OK, (nodes, backreferences)

def _create_minimal_graph(self, tokens):
    """Create a minimal valid AMR graph for failed cases"""
    from penman import Graph
    # Check if tokens suggest an interjection
    if any(t in {'...', '..', '.'} for t in map(str, tokens)):
        return Graph([('i', ':instance', 'interjection')])
    return Graph([('u', ':instance', 'unknown')])

def _create_linear_graph(self, nodes):
    """Create a linear chain graph from nodes"""
    from penman import Graph
    triples = []
    last = None
    for i, node in enumerate(nodes):
        if i == 0:
            triples.append(('top', ':instance', node[1] if len(node) > 1 else 'unknown'))
            last = 'top'
        else:
            new_node = f'n{i}'
            triples.append((new_node, ':instance', node[1] if len(node) > 1 else 'unknown'))
            triples.append((last, ':rel', new_node))
            last = new_node
    return Graph(triples)

def _simplify_references(self, nodes):
    """Simplify backreferences when pointer restoration fails"""
    simple_nodes = []
    backreferences = []
    for i, node in enumerate(nodes):
        if len(node) > 2 and isinstance(node[2], list):  # Has backreferences
            simple_nodes.append(node[:2])
        else:
...
class PENMANBartMultiGenTokenizer(AMRBartMultiGenTokenizer):
...
class AMRBartBaseTokenizer(BartTokenizer):
...
class PENMANBartBaseTokenizer(AMRBartBaseTokenizer):
