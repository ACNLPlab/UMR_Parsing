...
import spring_amr
spring_amr.ROOT = Path("/home/common/ACNLP/umr_parsing/BiBL")
ROOT = spring_amr.ROOT
...
from spring_amr.utils import instantiate_model_and_tokenizer, instantiate_loader, instantiate_model_and_tokenizer_multigen, instantiate_multitask_loader
...
def do_train(checkpoint=None, direction='amr', split_both_decoder=False, fp16=False):
    ...
    model, tokenizer = instantiate_model_and_tokenizer_multigen(
        config['model'],
    ...
    if checkpoint is not None:
        print(f'Checkpoint restored ({checkpoint})!', flush=True)
    ...
    train_loader = instantiate_multitask_loader(
        ...
        next_gen=False,
        amr_mask_ratio=config['amr_mask_ratio'],
        nl_mask_ratio=config['nl_mask_ratio']
    )

    #ROOT = "/home/common/ACNLP/umr_parsing/BiBL/"
    dev_gold_path = ROOT / config["dev_gold"]
    dev_pred_path = ROOT / config["dev_pred"]
    ...
    print("Dataset Preparation Over.")
    ...
    if direction == 'amr':
        ...
        def train_step(engine, batch):
            ...
            x, y, extra = batch
            model.amr_mode = True
            if config["use_lm_loss"] or config["use_mask_loss"]:
                skip_lm = extra["lm_batch"]["skip_lm"]
                skip_mask = extra["mask_batch"]["skip_mask"]
                if not skip_lm:
                    # print("Do Batch LM once")
                    lm_input = {
                        "input_ids": extra["lm_batch"]["input_ids"], 
                        "decoder_input_ids": extra["lm_batch"]["decoder_input_ids"],
                        "attention_mask": extra["lm_batch"]["attention_mask"],
                        "lm_labels": extra["lm_batch"]["lm_labels"]
                    }
                if not skip_mask:
                    # print("Do Batch LM once")
                    mask_input = {
                        "input_ids": extra["mask_batch"]["input_ids"], 
                        "decoder_input_ids": extra["mask_batch"]["decoder_input_ids"],
                        "attention_mask": extra["mask_batch"]["attention_mask"],
                        "lm_labels": extra["mask_batch"]["lm_labels"]
                    }
            disable_lm = False
            if config["use_lm_loss"] and (skip_lm or (len(lm_input["decoder_input_ids"]) > 0 and len(lm_input["decoder_input_ids"][0]) > config["batch_size"])):
                disable_lm = True
                print("Skip Batch LM once")
            disable_mask = False
            if config["use_mask_loss"] and (skip_mask or (len(lm_input["decoder_input_ids"]) > 0 and len(lm_input["decoder_input_ids"][0]) > config["batch_size"])):
                disable_mask = True
                print("Skip Batch Mask once")
            # print(x)
            # print(y)
            # print(lm_input)
            with autocast(enabled=fp16):
                classic_loss, *_ = model(**x, **y)
                loss = classic_loss
                if config["use_lm_loss"] and not disable_lm:
                    lm_loss, *_ = model(**lm_input)
                    loss = loss + config["lm_loss_weight"] * lm_loss
                if config["use_mask_loss"] and not disable_mask:
                    mask_loss, *_ = model(**mask_input)
                    loss = loss + config["mask_loss_weight"] * mask_loss
        ...
        def train_step(engine, batch):
            ...
            if config["use_lm_loss"] or config["use_mask_loss"]:
                ...
                if not skip_lm:
                    ...
                    lm_input = {
                        "input_ids": extra["lm_batch"]["input_ids"], 
                        "decoder_input_ids": extra["lm_batch"]["rev_decoder_input_ids"],
                        "attention_mask": extra["lm_batch"]["rev_attention_mask"],
                        "lm_labels": extra["lm_batch"]["rev_lm_labels"]
                    }
                if not skip_mask:
                    # print("Do Batch LM once")
                    mask_input = {
                        "input_ids": extra["mask_batch"]["rev_input_ids"], 
                        "decoder_input_ids": extra["mask_batch"]["rev_decoder_input_ids"],
                        "attention_mask": extra["mask_batch"]["rev_attention_mask"],
                        "lm_labels": extra["mask_batch"]["rev_lm_labels"]
                    }
            disable_lm = False
            if config["use_lm_loss"] and (skip_lm or (len(lm_input["decoder_input_ids"]) > 0 and len(lm_input["decoder_input_ids"][0]) > config["batch_size"])):
                disable_lm = True
